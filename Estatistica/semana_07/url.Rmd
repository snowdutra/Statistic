...
```{r}
acessorios = tabela[[1]] %>%
  rename(Itens = Acessórios, Tributação = '% Tributação')

acessorios$Tributação = gsub(",", ".", acessorios$Tributação)
acessorios$Tributação = gsub("%", "", acessorios$Tributação)
acessorios$Tributação = as.numeric (acessorios$Tributação)

acessorios = arrange(acessorios, desc(Tributação))
...
grafico <- plot_ly(acessorios, 
                   x = ~Itens, 
                   y = ~Tributação, 
                   type = 'bar',
                   marker = list(color = '#4CAF50')) %>%
  layout(title = "Tributação por Item",
