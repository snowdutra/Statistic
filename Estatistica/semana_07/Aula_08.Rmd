...
mapa <- leaflet() %>%
  ...
  addMarkers(
    ...
    popup=c("Moro aqui","Rio","POA","Paris"))  # escrever um comentário
mapa
...
mapasul = leaflet() %>%
  addTiles() %>%  
  addMarkers(
    lng = capital_sul$long ,  # inserir os dados com as longitudes
    lat = capital_sul$lat ,    # inserir os dados com as latitudes
...
mapa_populoso = leaflet() %>%
  ...
  addMarkers(
    lng = populosas$long,
    lat = populosas$lat,
    popup = paste0(1:nrow(populosas), "º ", populosas$name)
  )
mapa_populoso
...
mapa2 = ggplot() +
  ...
  geom_text( data=data %>% arrange(pop) %>% tail(10), 
                   aes(x=long, y=lat, label=name), size=5) +
  geom_point( data=data %>% arrange(pop) %>% tail(10), aes(x=long, y=lat), color="red", size=2) +
  theme_void() + coord_map() +
...
Colocar o Exercício 2 (aula 08) e o último mapa do Brasil em um dashboard.
